...
class WindowSelector:
    ...
    def __init__(self, parent, tracker, callback_ui, callback_status):
        ...
        ttk.Label(main_p, text="배치할 창들을 선택해 주세요.", style="Container.TLabel").pack(pady=(0, 5))
        ttk.Label(main_p, text="* 다중 선택: Ctrl+클릭 / Shift+클릭", style="Dim.TLabel").pack(pady=(0, 15))
        
        # 목록 프레임
        list_card = ttk.Frame(main_p, style="Container.TFrame", padding=2)
        list_card.pack(fill="both", expand=True)
        
        self.tree = ttk.Treeview(list_card, columns=("title"), show="headings", selectmode="extended")
        ...
        ttk.Button(btn_p, text="선택 창 배정", command=self._apply).pack(side="right", padx=5)
        ttk.Button(btn_p, text="새로고침", command=self._refresh_list).pack(side="right", padx=5)
        ttk.Button(btn_p, text="취소", command=self.dialog.destroy).pack(side="right", padx=5)
    ...
    def _apply(self):
        ...
        empty_slots = [i for i, h in enumerate(self.tracker.slot_hwnds) if h is None]
        ...
        for i, hwnd in enumerate(selected_hwnds):
            if i < len(empty_slots):
                for j, h in enumerate(self.tracker.slot_hwnds):
                    if h == hwnd: self.tracker.slot_hwnds[j] = None
                self.tracker.slot_hwnds[empty_slots[i]] = hwnd
                count += 1
